...
def group_group_collide(set_one, set_two):
    remove = set([])
    for item in set_one:
        if group_collide(set_two, item):
            remove.add(item)
    if len(remove) > 0:
        set_one.difference_update(remove)
    return len(remove)
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            canvas.draw_image(self.image, [self.image_center[0] * self.age, self.image_center[1]], self.image_size,
                              self.pos, self.image_size, self.angle)
            self.age += 1
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
...
def rock_spawner():
    if len(rock_group) < 12:
        ...
        if dist(my_ship.get_position(), rock_pos) > (my_ship.get_radius() * 3):
            rock_avel = random.random() * .2 - .1
            rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
            if score > 0:
                rock_vel[0] *= score * 0.5
                rock_vel[1] *= score * 0.5
